...
public class PolymarketRiskVisualizationConfigTests(ILogger logger) : BenchmarkTests<PolymarketRiskVisualizationConfigTests>(logger)
{
    ...
    public void VisualizerPnLHistoryTest()
    {
        ...
        var snapshots = new[]
        {
            new PolymarketPnLSnapshot(DateTimeOffset.UtcNow.AddHours(-3), 0, 0, 0, 0, 0, 10),
            new PolymarketPnLSnapshot(DateTimeOffset.UtcNow.AddHours(-2), 0, 0, 0, 0, 0, 25),
            new PolymarketPnLSnapshot(DateTimeOffset.UtcNow.AddHours(-1), 0, 0, 0, 0, 0, 15),
            new PolymarketPnLSnapshot(DateTimeOffset.UtcNow, 0, 0, 0, 0, 0, 40)
        };
    ...
    public void VisualizerPositionsTableTest()
    {
        ...
        var positions = new[]
        {
            new PolymarketPosition("token-yes")
            {
                Quantity = 100,
                AverageCostBasis = 0.60,
                CurrentPrice = 0.75
            },
            new PolymarketPosition("long-asset-name-here-test-xxx")
            {
                Quantity = 50,
