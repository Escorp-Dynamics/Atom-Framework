...
/// <summary>
/// Дифференциальный capture-тест: навигационный запрос настоящего Chromium против запроса
...
/// </remarks>
[CancelAfter(60000)]
public sealed class ChromiumDifferentialCaptureTests
{
    ...
    private static string? FindChromium() => ChromiumPaths.FirstOrDefault(File.Exists);

    [Test]
    public async Task NavigationHeadersMatchRealChromiumShape()
    {
        var chromium = FindChromium();
        if (chromium is null) Assert.Ignore("Chromium не найден в системе — capture-тест пропущен");

        var browserHead = await CaptureAsync(chromium, runClient: false);
        var clientHead = await CaptureAsync(chromium, runClient: true);

        var browserHeaders = ParseHeaders(browserHead);
        var clientHeaders = ParseHeaders(clientHead);

        // 1. Порядок имён заголовков обязан совпадать байт-в-байт по именам.
        ...
        AssertValue(clientHeaders, "sec-fetch-user", GetBrowserValue(browserHeaders, "sec-fetch-user"));
    ...
    private static async Task<string> CaptureAsync(string chromium, bool runClient)
    {
        ...
        try
        {
            ...
            {
                var start = new ProcessStartInfo(chromium)
                {
                    ArgumentList =
                    {
                        "--headless=new", "--disable-gpu", "--no-sandbox",
                        "--user-data-dir=" + Path.Combine(Path.GetTempPath(), "atom-net-capture-profile"),
                        "--dump-dom", url,
                    },
