...
public class PendingCommandCollectionTests
{
    ...
    {
        Command testCommand = new(1, new TestCommandParameters("module.command"), JsonContext.Default.Command, JsonTestContext.Default.TestCommandParameters);
    ...
    public void TestCannotAddCommandToClosedCollection()
    {
        PendingCommandCollection collection = new();
        collection.Close();
        Assert.That(() => collection.AddPendingCommand(new Command(1, new TestCommandParameters("test.command"), JsonContext.Default.Command, JsonTestContext.Default.TestCommandParameters)), Throws.InstanceOf<BiDiException>().With.Message.EqualTo("Cannot add command; pending command collection is closed"));
    }
    ...
    public void TestCannotAddCommandWithDuplicateId()
    {
        ...
        PendingCommandCollection collection = new();
        collection.AddPendingCommand(testCommand);
        Assert.That(() => collection.AddPendingCommand(new Command(1, new TestCommandParameters("test.command"), JsonContext.Default.Command, JsonTestContext.Default.TestCommandParameters)), Throws.InstanceOf<BiDiException>().With.Message.EqualTo("Could not add command with id 1, as id already exists"));
    }
    ...
    public void TestCannotClearCollectionUnlessClosed()
    {
        ...
        PendingCommandCollection collection = new();
        collection.AddPendingCommand(testCommand);
        Assert.That(() => collection.Clear(), Throws.InstanceOf<BiDiException>().With.Message.EqualTo("Cannot clear the collection while it can accept new incoming commands; close it with the Close method first"));
    }
