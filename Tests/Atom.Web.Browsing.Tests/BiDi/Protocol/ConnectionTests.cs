...
public class ConnectionTests
{
    ...
    private ReadOnlyMemory<byte> lastConnectionReceivedData = Memory<byte>.Empty;
    ...
    public void InitializeServer()
    {
        ...
        this.lastConnectionReceivedData = Memory<byte>.Empty;
        this.connectionReceiveSyncEvent.Reset();
        this.serverReceiveSyncEvent.Reset();
        this.connectionSyncEvent.Reset();
        this.server = new Server();
        clientConnectedObserver = server.OnClientConnected.AddObserver(OnClientConnected);
        this.server.Start();
    }
    ...
    public void TestConnectionFailure()
    {
        ...
        {
            throw new BiDiException("No server available");
        }
        ...
        Assert.That(async () => await connection.StartAsync(new Uri($"ws://localhost:{port}")), Throws.InstanceOf<TimeoutException>().With.Message.Contains($"{0.25} seconds"));
    }
    ...
    public async Task TestConnectionCanSendData()
    {
        ...
        Connection connection = new();
        await connection.StartAsync(new Uri($"ws://localhost:{this.server.Port}"));
    ...
    public async Task TestConnectionCanReceiveData()
    {
        ...
        byte[] dataReceivedByConnection = this.WaitForConnectionToReceiveData(TimeSpan.FromSeconds(3)).ToArray();
    ...
    public async Task TestConnectionReceivesDataOnBufferBoundary()
    {
        ...
        Connection connection = new();
        await connection.StartAsync(new Uri($"ws://localhost:{server.Port}"));
        string registeredConnectionId = this.WaitForServerToRegisterConnection(TimeSpan.FromSeconds(1));
        connection.OnDataReceived.AddObserver(OnConnectionDataReceivedAsync);
        ...
        var dataReceivedByConnection = this.WaitForConnectionToReceiveData(TimeSpan.FromSeconds(3)).ToArray();
    ...
    public async Task TestConnectionReceivesDataOnVeryLongMessage()
    {
        ...
        await this.server.SendDataAsync(registeredConnectionId, data);
        var dataReceivedByConnection = WaitForConnectionToReceiveData(TimeSpan.FromSeconds(3));
        var data2 = Encoding.UTF8.GetString(dataReceivedByConnection);
    ...
    public async Task TestConnectionLog()
    {
        ...
        foreach (LogMessageEventArgs args in logValues)
        {
            Assert.Multiple(() =>
            {
                Assert.That(args.Level, Is.EqualTo(BiDiLogLevel.Info));
    ...
    public async Task TestUrlProperty()
    {
        ...
        var serverWebSocketUrl = new Uri($"ws://localhost:{this.server.Port}");
        var connection = new Connection();
        Assert.That(connection.ConnectedUrl, Is.Null);
        connection.OnDataReceived.AddObserver(OnConnectionDataReceivedAsync);
    ...
    public async Task TestConnectionCanBeReusedAfterBeingShutDown()
    {
        ...
        byte[] receivedData = this.WaitForConnectionToReceiveData(TimeSpan.FromMilliseconds(250)).ToArray();
        await connection.StopAsync();
        ...
        receivedData = this.WaitForConnectionToReceiveData(TimeSpan.FromMilliseconds(250)).ToArray();
        await connection.StopAsync();
    ...
    public async Task TestConnectionCanBeReusedAfterBeingAborted()
    {
        ...
        await this.server.SendDataAsync(registeredConnectionId, "First connection acknowledged");
        var receivedData = this.WaitForConnectionToReceiveData(TimeSpan.FromMilliseconds(250)).ToArray();
        this.server.IgnoreCloseConnectionRequest(registeredConnectionId, true);
        await connection.StopAsync();
        Assert.That(receivedData, Is.EqualTo("First connection acknowledged"u8.ToArray()));
    ...
    public async Task TestCannotStartAlreadyStartedConnection()
    {
        ...
        await connection.StartAsync(new Uri($"ws://localhost:{this.server.Port}"));
        this.WaitForServerToRegisterConnection(TimeSpan.FromSeconds(1));
        Assert.That(async () => await connection.StartAsync(new Uri($"ws://localhost:{this.server.Port}")), Throws.InstanceOf<BiDiException>().With.Message.StartsWith($"The WebSocket is already connected to ws://localhost:{this.server.Port}"));
    }
    ...
    public void TestCannotSendDataOnAConnectionNotYetStarted()
    {
        ...
        Assert.That(async () => await connection.SendDataAsync("This send should fail"u8.ToArray()), Throws.InstanceOf<BiDiException>().With.Message.StartsWith($"The WebSocket has not been initialized"));
    }
    ...
    public async Task TestDataSendOperationsAreSynchronized()
    {
        ...
        syncEvent.Wait();
        Assert.That(async () => await connection.SendDataAsync("second data"u8.ToArray()), Throws.InstanceOf<BiDiException>().With.Message.EqualTo("Timed out waiting to access WebSocket for sending; only one send operation is permitted at a time."));
        await connection.StopAsync();
    }
    ...
    private ValueTask OnConnectionDataReceivedAsync(ConnectionDataReceivedEventArgs e)
    {
    ...
    private ReadOnlySpan<byte> WaitForConnectionToReceiveData(TimeSpan timeout)
    {
        this.connectionReceiveSyncEvent.WaitOne(timeout);
        return this.lastConnectionReceivedData.Span;
    }
