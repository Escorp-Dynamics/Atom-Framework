...
public class CaptureScreenshotCommandParametersTests
{
    ...
    {
        CaptureScreenshotCommandParameters properties = new("myContextId");
        string json = JsonSerializer.Serialize(properties, JsonContext.Default.CaptureScreenshotCommandParameters);
    ...
    public void TestImageFormatQualityParameterOutsideValidRangeThrows()
    {
        ...
        Assert.That(() => format.Quality = -.01, Throws.InstanceOf<BiDiException>().With.Message.EqualTo("Качество должно быть в диапазоне от 0 до 1 включительно"));
        Assert.That(() => format.Quality = 1.01, Throws.InstanceOf<BiDiException>().With.Message.EqualTo("Качество должно быть в диапазоне от 0 до 1 включительно"));
    }
}