...
public class BrowsingContextModuleTests
{
    ...
    public async Task TestExecuteGetTreeCommand()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(result.ContextTree.First().BrowsingContextId, Is.EqualTo("myContext"));
            Assert.That(result.ContextTree.First().Url.OriginalString, Is.EqualTo("https://example.com"));
            Assert.That(result.ContextTree.First().Children.ToArray(), Has.Length.EqualTo(0));
        });
    }
    ...
    public async Task TestExecuteNavigateCommand()
    {
        ...
        var task = module.NavigateAsync(new NavigateCommandParameters("myContext", new Uri("https://example.com")) { Wait = ReadinessState.Complete });
        task.AsTask().Wait(TimeSpan.FromSeconds(1));
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(result.Url.OriginalString, Is.EqualTo("https://example.com"));
        });
    }
    ...
    public async Task TestCanReceiveContextCreatedEvent()
    {
        ...
        module.OnContextCreated.AddObserver((BrowsingContextEventArgs e) => {
            Assert.Multiple(() =>
            {
                ...
                Assert.That(e.Url.OriginalString, Is.EqualTo("https://example.com"));
                Assert.That(e.Children.ToArray(), Has.Length.EqualTo(0));
