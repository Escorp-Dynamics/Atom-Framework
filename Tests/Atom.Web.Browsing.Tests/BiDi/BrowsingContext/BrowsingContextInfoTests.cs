...
public class BrowsingContextInfoTests
{
    ...
    public void TestCanDeserialize()
    {
        string json = """
                      ...
                      """;
        BrowsingContextInfo? info = JsonSerializer.Deserialize(json, JsonContext.Default.BrowsingContextInfo);
        Assert.That(info, Is.Not.Null);
        Assert.That(info, Is.InstanceOf<BrowsingContextInfo>());
        Assert.Multiple(() =>
        {
            Assert.That(info!.BrowsingContextId, Is.EqualTo("myContextId"));
            Assert.That(info.Url, Is.EqualTo(new Uri("http://example.com")));
    ...
    public void TestCanDeserializeWithChildren()
    {
        string json = """
                      ...
                      """;
        BrowsingContextInfo? info = JsonSerializer.Deserialize<BrowsingContextInfo>(json, JsonContext.Default.BrowsingContextInfo);
        Assert.That(info, Is.Not.Null);
        Assert.That(info, Is.InstanceOf<BrowsingContextInfo>());
        Assert.Multiple(() =>
        {
            ...
            Assert.That(info.ClientWindowId, Is.EqualTo("myClientWindowId"));
            Assert.That(info.OriginalOpener, Is.EqualTo("openerContext"));
            Assert.That(info.Children.ToArray(), Has.Length.EqualTo(1));
            Assert.That(info.Parent, Is.Null);
        });
    }
    ...
    public void TestCanDeserializeWithOptionalParent()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(info.ClientWindowId, Is.EqualTo("myClientWindowId"));
            Assert.That(info.OriginalOpener, Is.EqualTo("openerContext"));
            Assert.That(info.Children.ToArray(), Has.Length.EqualTo(0));
    ...
    public void TestDeserializingBrowsingContextInfoWithMissingContextThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<BrowsingContextInfo>(json, JsonContext.Default.BrowsingContextInfo), Throws.InstanceOf<JsonException>());
    }
