...
public class InitiatorTests
{
    ...
    public void TestCanDeserializeInitiator()
    {
        string json = """
                      ...
                      """;
        Initiator? initiator = JsonSerializer.Deserialize<Initiator>(json, JsonContext.Default.Initiator);
    ...
    public void TestDeserializingInitiatorWithInvalidTypeValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<Initiator>(json, JsonContext.Default.Initiator), Throws.InstanceOf<BiDiException>().With.Message.Contains("значение 'invalid' недопустимо для типа перечисления"));
    }
}
