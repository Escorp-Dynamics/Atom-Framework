   ...
   [Test]
    public void TestCanDeserializeStringRemoteValue()
    {
        string json = """
                      ...
                      """;
        RemoteValue? remoteValue = JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue);
    ...
    public void TestDeserializingInvalidStringRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("должно быть непустой строкой"));
    }
    ...
    public void TestDeserializingInvalidSpecialNumericRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("недопустимое значение 'invalid' для свойства 'value' числа"));
    }
    ...
    public void TestDeserializingInvalidNumericRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("недопустимый тип Boolean для свойства 'value' числа"));
        json = """
               {
                 "type": "number",
                 "value": false
        ...
        json = """
               ...
               """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("недопустимый тип Null для свойства 'value' числа"));
    }
    ...
    public void TestDeserializingInvalidBooleanRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("Свойство 'value' в RemoteValue для boolean должно быть логическим значением"));
    }
    ...
    public void TestDeserializingInvalidBigIntRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("bigint должен содержать непустое свойство 'value', значение которого является строкой"));
    }
    ...
    public void TestDeserializingInvalidBigIntValueRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("RemoteValue не может разобрать недопустимое значение 'some value' для bigint"));
    }
    ...
    public void TestDeserializingInvalidDateRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("RemoteValue для date должен содержать непустое свойство 'value', значение которого является строкой"));
    }
    ...
    public void TestDeserializingInvalidDateValueRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("RemoteValue не может разобрать недопустимое значение 'some value' для date"));
    }
    ...
    public void TestDeserializingInvalidRegularExpressionValueRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("regexp должен содержать непустое свойство 'value', значение которого является объектом"));
    }
    ...
    public void TestDeserializingNodeRemoteValueWithInvalidSharedIdThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("Свойство 'sharedId' в RemoteValue, если присутствует, должно быть строкой"));
    }
    ...
    public void TestDeserializingInvalidNodeValueRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("node должен содержать непустое свойство 'value', значение которого является объектом"));
    }
    ...
    public void TestDeserializingInvalidArrayValueRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("array должен содержать непустое свойство 'value', значение которого является массивом"));
    }
    ...
    public void TestDeserializingInvalidArrayElementValueRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("Каждый элемент списка в RemoteValue должен быть объектом"));
    }
    ...
    public void TestDeserializingInvalidSetValueRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("set должен содержать непустое свойство 'value', значение которого является массивом"));
    }
    ...
    public void TestDeserializingInvalidNodeListValueRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("nodelist должен содержать непустое свойство 'value', значение которого является массивом"));
    }
    ...
    public void TestDeserializingInvalidHtmlCollectionValueRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("htmlcollection должен содержать непустое свойство 'value', значение которого является массивом"));
    }
    ...
    public void TestDeserializingInvalidMapValueRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("map должен содержать непустое свойство 'value', значение которого является массивом"));
    }
    ...
    public void TestDeserializingMapRemoteValueWithoutArrayElementsThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("Элемент массива для словаря в RemoteValue должен быть массивом"));
    }
    ...
    public void TestDeserializingMapRemoteValueWithInvalidKeyTypeThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("Первый элемент (ключ) массива для словаря в RemoteValue должен быть строкой или объектом"));
    }
    ...
    public void TestDeserializingMapRemoteValueWithInvalidValueTypeThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("Второй элемент (значение) массива для словаря в RemoteValue должен быть объектом"));
    }
    ...
    public void TestDeserializingInvalidObjectValueRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("object должен содержать непустое свойство 'value', значение которого является массивом"));
    }
    ...
    public void TestDeserializingInvalidWindowValueRemoteValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("window должен содержать непустое свойство 'value', значение которого является объектом"));
    }
    ...
    public void TestDeserializingWindowRemoteValueWithInvalidHandleTypeThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>());
    }
    ...
    public void TestDeserializingRemoteValueWithMissingTypeThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("должен содержать свойство 'type'"));
    }
    ...
    public void TestDeserializingRemoteValueWithInvalidTypeThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("Свойство 'type' в RemoteValue должно быть строкой"));
    }
    ...
    public void TestDeserializingRemoteValueWithInvalidTypeValueThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("Значение свойства 'type' в RemoteValue 'invalid' не является допустимым типом RemoteValue"));
    }
    ...
    public void TestDeserializingRemoteValueWithEmptyStringTypeThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("RemoteValue должен содержать непустое свойство 'type', которое является строкой"));
    }
    ...
    public void TestValueAsWithIncorrectType()
    {
        ...
        {
            Assert.That(remoteValue, Is.Not.Null);
            Assert.That(() => remoteValue!.ValueAs<string>(), Throws.InstanceOf<BiDiException>().With.Message.EqualTo("RemoteValue could not be cast to the desired type"));
        });
    }
    ...
    public void TestNullRemoteValueAsValueType()
    {
        ...
        {
            Assert.That(remoteValue, Is.Not.Null);
            Assert.That(() => remoteValue!.ValueAs<int>(), Throws.InstanceOf<BiDiException>().With.Message.EqualTo("RemoteValue has null value, but desired type is a value type"));
        });
    }
    ...
    public void TestDeserializingNonObjectThrows()
    {
        ...
        Assert.That(() => JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue), Throws.InstanceOf<JsonException>().With.Message.Contains("RemoteValue JSON должен быть объектом, но был Array"));
    }
    ...
    public void TestCannotSerialize()
    {
        string json = """
                      ...
                      """;
        RemoteValue? remoteValue = JsonSerializer.Deserialize<RemoteValue>(json, JsonContext.Default.RemoteValue);
        Assert.That(() => JsonSerializer.Serialize(remoteValue!), Throws.InstanceOf<InvalidOperationException>());
    }
    ...
    public void TestConvertToRemoteReferenceWithoutHandleThrows()
    {
        ...
        {
            Assert.That(remoteValue, Is.Not.Null);
            Assert.That(() => remoteValue!.ToRemoteReference(), Throws.InstanceOf<BiDiException>().With.Message.Contains("must have a valid handle"));
        });
    }
    ...
    public void TestConvertNodeRemoteValueWithoutSharedIdToRemoteReferenceThrows()
    {
        ...
        {
            Assert.That(remoteValue, Is.Not.Null);
            Assert.That(() => remoteValue!.ToRemoteReference(), Throws.InstanceOf<BiDiException>().With.Message.Contains("must have a valid shared ID"));
        });
    }
    ...
    public void TestConvertPrimitiveRemoteValueToRemoteReferenceThrows()
    {
        ...
        {
            Assert.That(remoteValue, Is.Not.Null);
            Assert.That(() => remoteValue!.ToRemoteReference(), Throws.InstanceOf<BiDiException>().With.Message.Contains("Primitive values cannot be used as remote references"));
        });
    }
    ...
    public void TestConvertNonNodeRemoteValueToSharedReferenceThrows()
    {
        ...
        {
            Assert.That(remoteValue, Is.Not.Null);
            Assert.That(() => remoteValue!.ToSharedReference(), Throws.InstanceOf<BiDiException>().With.Message.Contains("Remote value cannot be converted to SharedReference"));
        });
     }
}
