...
public class ReceivedDataDictionaryTests
{
    ...
    {
        Assert.That(ReceivedDataDictionary.Empty, Is.Empty);
    }
    ...
    public void TestCanCreateFromDeserializedData()
    {
        ...
        OnlyOverflowData? deserializedValue = JsonSerializer.Deserialize<OnlyOverflowData>(json, JsonTestContext.Default.OnlyOverflowData);
        Assert.That(deserializedValue!.ReceivedData, Has.Count.EqualTo(8));
    }
}

public class OnlyOverflowData
{
    private Dictionary<string, JsonElement> overflowData = new();
    private ReceivedDataDictionary receivedData = ReceivedDataDictionary.Empty;
    ...
    [JsonExtensionData]
    [JsonInclude]
    public Dictionary<string, JsonElement> OverflowData { get => this.overflowData; set => this.overflowData = value; }
}