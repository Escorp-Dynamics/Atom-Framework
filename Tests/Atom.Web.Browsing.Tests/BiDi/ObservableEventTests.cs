namespace Atom.Web.Browsing.BiDi.Tests;
...
public class ObservableEventTests
{
    ...
    public void TestCannotAddMoreThanMaxObservers()
    {
        ...
        Assert.That(() => testEventSource.TestObservableEvent.AddObserver((TestObservableEventArgs e) => _ = e.EventValue), Throws.InstanceOf<BiDiException>().With.Message.EqualTo("Это наблюдаемое событие позволяет только 1 обработчик"));
        ...
        Assert.That(() => testEventSource.TestObservableEvent.AddObserver((TestObservableEventArgs e) => _ = e.EventValue), Throws.InstanceOf<BiDiException>().With.Message.EqualTo("Это наблюдаемое событие позволяет только 2 обработчиков"));
    }
    [Test]
