...
public class ProxyConfigurationTests
{
    ...
    public void TestCanSerializeWithHttpProxy()
    {
        ...
        string json = JsonSerializer.Serialize(proxy, JsonContext.Default.ManualProxyConfiguration);
    ...
    public void TestCanSerializeWithProxyTypeDirect()
    {
        ...
        string json = JsonSerializer.Serialize(proxy, JsonContext.Default.DirectProxyConfiguration);
        JObject serialized = JObject.Parse(json);
    ...
    public void TestCanSerializeWithProxyTypeSystem()
    {
        ...
        string json = JsonSerializer.Serialize(proxy, JsonContext.Default.SystemProxyConfiguration);
        JObject serialized = JObject.Parse(json);
        Assert.That(serialized, Has.Count.EqualTo(1));
        Assert.Multiple(() =>
        {
    ...
    public void TestCanSerializeWithProxyTypeAutoDetect()
    {
        ...
        string json = JsonSerializer.Serialize(proxy, JsonContext.Default.AutoDetectProxyConfiguration);
        JObject serialized = JObject.Parse(json);
        Assert.That(serialized, Has.Count.EqualTo(1));
        Assert.Multiple(() =>
        {
    ...
    {
        ProxyConfiguration proxy = new PacProxyConfiguration(new Uri("http://proxy.autoconfig.url"));
        string json = JsonSerializer.Serialize(proxy, JsonContext.Default.PacProxyConfiguration);
        JObject serialized = JObject.Parse(json);
        Assert.That(serialized, Has.Count.EqualTo(2));
        Assert.Multiple(() =>
        {
            ...
            Assert.That(serialized["proxyAutoconfigUrl"]!.Value<string>(), Is.EqualTo("http://proxy.autoconfig.url"));
        });
    }
    ...
    public void TestCanDeserializeWithProxyTypeManual()
    {
        string json = """
                      ...
                      """;
        ProxyConfiguration? deserialized = JsonSerializer.Deserialize(json, JsonContext.Default.ManualProxyConfiguration);
    ...
    public void TestCanDeserializeWithProxyTypeSystem()
    {
        string json = """
                      ...
                      """;
        ProxyConfiguration? deserialized = JsonSerializer.Deserialize(json, JsonContext.Default.SystemProxyConfiguration);
        Assert.That(deserialized, Is.Not.Null);
        Assert.Multiple(() =>
        {
    ...
    public void TestCanDeserializeWithProxyTypeAutoDetect()
    {
        string json = """
                      ...
                      """;
        ProxyConfiguration? deserialized = JsonSerializer.Deserialize(json, JsonContext.Default.AutoDetectProxyConfiguration);
        Assert.That(deserialized, Is.Not.Null);
        Assert.Multiple(() =>
        {
    ...
    public void TestCanDeserializeWithProxyTypeProxyAutoconfig()
    {
        string json = """
                      ...
                      """;
        ProxyConfiguration? deserialized = JsonSerializer.Deserialize(json, JsonContext.Default.PacProxyConfiguration);
        Assert.That(deserialized, Is.Not.Null);
        Assert.Multiple(() =>
        {
            ...
            Assert.That(deserializedResult.ProxyAutoConfigUrl.OriginalString, Is.EqualTo("proxy.autoconfig.url"));
        });
    }
    ...
    public void TestCanDeserializeWithProxyTypeDirect()
    {
        string json = """
                      ...
                      """;
        ProxyConfiguration? deserialized = JsonSerializer.Deserialize(json, JsonContext.Default.DirectProxyConfiguration);
        Assert.That(deserialized, Is.Not.Null);
        Assert.Multiple(() =>
        {
    ...
    public void TestDeserializeWithNonObjectJsonThrows()
    {
        ...
        Assert.That(() => JsonSerializer.Deserialize(json, JsonContext.Default.ProxyConfiguration), Throws.InstanceOf<JsonException>().With.Message.Contains("прокси должен быть объектом"));
    }
    ...
    public void TestDeserializeWithProxyTypeProxyAutoconfigWithMissingUrlThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize(json, JsonContext.Default.ProxyConfiguration), Throws.InstanceOf<JsonException>().With.Message.Contains("proxyAutoconfigUrl"));
    }
    ...
    public void TestDeserializeWithInvalidProxyTypeThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize(json, JsonContext.Default.ProxyConfiguration), Throws.InstanceOf<BiDiException>().With.Message.Contains("значение 'invalid' недопустимо для типа перечисления"));
    }
    ...
    public void TestDeserializeWithNonStringProxyTypeThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize(json, JsonContext.Default.ProxyConfiguration), Throws.InstanceOf<JsonException>().With.Message.Contains("должно быть строкой"));
    }
}
