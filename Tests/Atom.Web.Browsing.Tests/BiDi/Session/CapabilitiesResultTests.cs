...
public class CapabilitiesResultTests
{
    ...
    public void TestCanDeserialize()
    {
        string json = """
                      ...
                      """;
        CapabilitiesResult? result = JsonSerializer.Deserialize<CapabilitiesResult>(json, JsonContext.Default.CapabilitiesResult);
        Assert.That(result, Is.Not.Null);
        Assert.Multiple(() =>
        {
            Assert.That(result!.IsAcceptInsecureCertificates, Is.EqualTo(true));
            ...
            Assert.That(result.IsSetWindowRect, Is.EqualTo(true));
    ...
    public void TestCanDeserializeWithWebSocketUrl()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(result!.UnhandledPromptBehavior, Is.Null);
            Assert.That(result.IsSetWindowRect, Is.EqualTo(true));
            Assert.That(result.WebSocketUrl?.OriginalString, Is.EqualTo("ws://socket:1234"));
            Assert.That(result.AdditionalCapabilities, Contains.Key("capName"));
            Assert.That(result.AdditionalCapabilities["capName"], Is.EqualTo("capValue"));
        });
    }
    ...
    public void TestCanDeserializeManualWithFullProxyProperties()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(proxyResult.NoProxyAddresses?.ToArray(), Has.Length.EqualTo(1));
    ...
    public void TestCanDeserializeProxyAutoConfigWithFullProxyProperties()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(proxyResult.ProxyAutoConfigUrl.OriginalString, Is.EqualTo("proxy.autoconfig.url"));
            Assert.That(proxyResult.AdditionalData, Is.Empty);
            Assert.That(result.IsSetWindowRect, Is.EqualTo(true));
            Assert.That(result.AdditionalCapabilities, Contains.Key("capName"));
            Assert.That(result.AdditionalCapabilities["capName"], Is.EqualTo("capValue"));
    ...
    public void TestCanDeserializeWithInvalidProxyTypeThrows()
    {
        ...
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<CapabilitiesResult>(json, JsonContext.Default.CapabilitiesResult), Throws.InstanceOf<BiDiException>().With.Message.Contains("значение 'proxyautoconfig' недопустимо для типа перечисления"));
    ...
    public void TestCannotDeserializeWithEmptyProxy()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<CapabilitiesResult>(json, JsonContext.Default.CapabilitiesResult), Throws.InstanceOf<JsonException>().With.Message.Contains("must contain a 'proxyType' property"));
    }
    ...
    public void TestCanDeserializingWithInvalidUnhandledPromptBehaviorThrows()
    {
        string json = """
                      ...
                      """;
        Assert.That(() => JsonSerializer.Deserialize<CapabilitiesResult>(json, JsonContext.Default.CapabilitiesResult), Throws.InstanceOf<JsonException>());
    }
