...
public class ServiceProxyTests(ILogger logger) : BenchmarkTests<ServiceProxyTests>(logger)
{
    ...
    public void SerializeTest()
    {
        ...
        var proxy2 = ServiceProxy.Deserialize(json) as ServiceProxy;

        if (!IsBenchmarkEnabled)
        {
            Assert.That(proxy2, Is.Not.Null);
            if (proxy2 is null) return;
        ...
        {
            Assert.That(json, Is.Not.Null);
            Assert.That(json, Is.EqualTo(/*lang=json,strict*/ "{\"host\":\"localhost\",\"port\":80,\"anonymity\":3,\"geolocation\":{\"country\":\"BRA\",\"latitude\":45.65656,\"longitude\":135.6334544},\"asn\":\"Test ASN\",\"alive\":\"01.02.2025 21:04:35\",\"uptime\":40}"));
        }

        if (string.IsNullOrEmpty(json)) return;
        proxy2 = ServiceProxy.Deserialize(json) as ServiceProxy;
