...
public class PolymarketClientTests(ILogger logger) : BenchmarkTests<PolymarketClientTests>(logger)
{
    ...
    public async Task NoSubscribersNoErrorTest()
    {
        ...
        byte[] json = """{"event_type":"book","asset_id":"a1","market":"m1","buys":[],"sells":[]}"""u8.ToArray();

        Assert.DoesNotThrowAsync(async () =>
            await client.ProcessMessageAsync(json, PolymarketChannel.Market));
    }
    ...
    public async Task MultipleSequentialMessagesTest()
    {
        ...
        byte[] bookJson = """{"event_type":"book","asset_id":"a1","market":"m1","buys":[],"sells":[]}"""u8.ToArray();
        byte[] priceJson = """{"event_type":"price_change","asset_id":"a1","market":"m1","changes":[]}"""u8.ToArray();

        await client.ProcessMessageAsync(bookJson, PolymarketChannel.Market);
        await client.ProcessMessageAsync(priceJson, PolymarketChannel.Market);
        await client.ProcessMessageAsync(bookJson, PolymarketChannel.Market);
