...
/// <summary>
/// Контрактные тесты для KuCoin-реализации Markets/ интерфейсов.
/// </summary>
public class KuCoinMarketsContractTests(ILogger logger) : BenchmarkTests<KuCoinMarketsContractTests>(logger)
{
    public KuCoinMarketsContractTests() : this(ConsoleLogger.Unicode) { }

    #region Модели — IMarketPriceSnapshot

    [TestCase(TestName = "KuCoin IMarketPriceSnapshot: все свойства доступны")]
    public void PriceSnapshotImplementsInterface()
    {
        var snap = new KuCoinPriceSnapshot
        {
            AssetId = "BTC-USDT",
            BestBid = 65000.5,
            BestAsk = 65001.0,
    ...
    [TestCase(TestName = "KuCoin IMarketPriceSnapshot: null значения")]
    public void PriceSnapshotNullValues()
    {
        var snap = new KuCoinPriceSnapshot { AssetId = "ETH-USDT" };
        IMarketPriceSnapshot isnap = snap;
    ...
    [TestCase(TestName = "KuCoin IMarketPosition: все свойства и вычисления")]
    public void PositionImplementsInterface()
    {
        var pos = new KuCoinPosition
        {
            AssetId = "BTC-USDT",
            Quantity = 0.5,
    ...
    [TestCase(TestName = "KuCoin IMarketPosition: IsClosed = true при нулевом количестве")]
    public void PositionIsClosedTest()
    {
        var pos = new KuCoinPosition { AssetId = "ETH-USDT", Quantity = 0 };
    ...
    [TestCase(TestName = "KuCoin IMarketPortfolioSummary: NetPnL = unrealized + realized - fees")]
    public void PortfolioSummaryNetPnL()
    {
        var summary = new KuCoinPortfolioSummary
        {
    ...
    [TestCase(TestName = "KuCoin IMarketOrderBookSnapshot: AssetId и Timestamp")]
    public void OrderBookSnapshotImplementsInterface()
    {
        var ts = DateTimeOffset.UtcNow;
        var book = new KuCoinOrderBookSnapshot
        {
            AssetId = "BTC-USDT",
            Timestamp = ts,
            Bids = [(65000, 1.5)],
    ...
    [TestCase(TestName = "KuCoin IMarketTradeSignal: все свойства доступны")]
    public void TradeSignalImplementsInterface()
    {
        var signal = new KuCoinTradeSignal
        {
            AssetId = "BTC-USDT",
    ...
    [TestCase(TestName = "KuCoinException наследует MarketException")]
    public void ExceptionInheritance()
    {
        var ex = new KuCoinException("test error");
    ...
    [TestCase(TestName = "KuCoin IMarketClient: PlatformName = 'KuCoin'")]
    public void ClientPlatformName()
    {
        ...
        using var scope = Assert.EnterMultipleScope();
        Assert.That(iclient.PlatformName, Is.EqualTo("KuCoin"));
    ...
    [TestCase(TestName = "KuCoin IMarketRestClient: BaseUrl корректный")]
    public void RestClientBaseUrl()
    {
        using var rest = new KuCoinRestClient();
        IMarketRestClient irest = rest;
        Assert.That(irest.BaseUrl, Is.EqualTo(KuCoinRestClient.DefaultApiUrl));
    }

    [TestCase(TestName = "KuCoin IMarketRestClient: кастомный BaseUrl")]
    public void RestClientCustomBaseUrl()
    {
        using var rest = new KuCoinRestClient("https://custom.kucoin.com");
        Assert.That(rest.BaseUrl, Is.EqualTo("https://custom.kucoin.com"));
    }
    ...
    [TestCase(TestName = "KuCoin IMarketPriceStream: кеш цен работает")]
    public void PriceStreamCacheWorks()
    {
        using var stream = new KuCoinPriceStream();
        ...
        stream.UpdatePrice("BTC-USDT", 65000, 65001, 65000.5);

        using var scope = Assert.EnterMultipleScope();
        Assert.That(istream.TokenCount, Is.EqualTo(1));
        Assert.That(istream.GetPrice("BTC-USDT")!.BestBid, Is.EqualTo(65000));
