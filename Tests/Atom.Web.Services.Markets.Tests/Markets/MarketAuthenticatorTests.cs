...
public class MarketAuthenticatorTests(ILogger logger) : BenchmarkTests<MarketAuthenticatorTests>(logger)
{
    ...
    public void HtxAuthenticatorFactory()
    {
        ...
        Assert.That(auth.AlgorithmName, Does.Contain("HMAC-SHA256"));
    ...
    [TestCase(TestName = "MarketStreamingPipeline: publish обновляет кеш и генерирует сигнал")]
    public async Task PipelinePublishUpdatesWritableStream()
    {
        using var client = new StubMarketClient("Test");
        using var priceStream = new StubPriceStream();
        await using var pipeline = new MarketStreamingPipeline(client, priceStream,
            config: new StreamingPipelineConfig { MinConfidence = 0.0, DryRun = true });

        pipeline.AddStrategy(new LiveSignalStrategy(), ["BTC"]);

        PipelineSignal? captured = null;
        pipeline.OnSignalGenerated += signal => captured = signal;

        await pipeline.StartAsync(["BTC"]);
        Assert.That(pipeline.PublishPriceUpdate(new PriceUpdate("BTC", 100, 102, 101, Environment.TickCount64)), Is.True);
        ...
        Assert.That(priceStream.GetPrice("BTC"), Is.Not.Null);
        Assert.That(captured, Is.Not.Null);
        Assert.That(captured!.Value.Signal.Action, Is.EqualTo(TradeAction.Buy));
    }
    ...
    private sealed class StubPriceStream : IWritableMarketPriceStream
    {
    ...
    private sealed class LiveSignalStrategy : IMarketStrategy
    {
        public string Name => "LiveSignal";
        ...
        public IMarketTradeSignal Evaluate(IMarketPriceStream priceStream, string assetId)
        {
            ...
            if (snapshot?.LastTradePrice is not { } price)
                return MarketTradeSignal.Hold(assetId);
