    ...
    where T : new()
{
    ...
    protected virtual void WriteProperty<TValue>([NotNull] Utf8JsonWriter writer, string propertyName, TValue? value, [NotNull] JsonSerializerOptions options, JsonTypeInfo<T>? typeInfo)
    {
    ...
    /// <param name="value">Значение свойства.</param>
    /// <param name="options">Опции сериализации JSON.</param>
    protected void WriteProperty<TValue>([NotNull] Utf8JsonWriter writer, string propertyName, TValue? value, [NotNull] JsonSerializerOptions options)
        => WriteProperty(writer, propertyName, value, options, default);
    ...
    private static bool ShouldIgnoreValue<TValue>(JsonSerializerOptions options, TValue? value)
        => options.DefaultIgnoreCondition is JsonIgnoreCondition.WhenWritingDefault or JsonIgnoreCondition.WhenWritingNull && value is null;
}