...
/// (<c lang="text">code = (code + blCount[bits-1]) &lt;&lt; 1</c>), а таблица индексируется развёрнутыми битами.
...
internal static class ZstdHuffman
{
    ...
    /// </summary>
    /// <remarks>
    /// RFC 8878, §4.2.1: сумма <c lang="text">2^(вес-1)</c> по известным весам дополняется до ближайшей степени
    /// двойки; недостающая часть обязана сама быть степенью двойки и задаёт последний вес,
    /// а полная степень — <c lang="text">Max_Number_of_Bits</c>.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// RFC 8878, §3.1.1.3.1.6: размер каждого сегмента равен <c lang="text">(Regenerated_Size+3)/4</c>,
