...
/// в <c>ZstdDecoder.ParseFseTable</c> и в <c>ZstdWeightsParser.ParseFseHeader</c>, и обе копии
...
/// <list type="number">
///   <item>
///     порог коротких кодов считался как <c>(1 &lt;&lt; (bits+1)) - range</c> — на единицу больше
///     эталонного <c>max = (2*threshold - 1) - remaining</c>, из-за чего одно значение читалось
///     на бит короче и весь дальнейший поток рассинхронизировался;
///   </item>
///   <item>
///     дополнительный бит приклеивался младшим разрядом (<c>(value &lt;&lt; 1) | extra</c>), хотя поток
...
///   </item>
///   <item>
///     длина описания возвращалась как <c>BitReader.BytesConsumed</c> — это позиция предзагрузки
...
internal static class ZstdFseTable
{
    ...
    /// </summary>
    /// <remarks>
    /// Собственный, потому что <c>Atom.IO.BitReader</c> при нехватке бит молча укорачивает
    /// запрошенное количество и не даёт узнать логическую позицию без учёта предзагрузки.
    /// Эталон (<c>FSE_readNCount</c>) точно так же дочитывает описание таблицы «сквозь» границу
