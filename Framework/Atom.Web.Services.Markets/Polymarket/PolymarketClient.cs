        ...
        try
        {
            ...
            if (marketSocket is null || marketSocket.State != WebSocketState.Open)
            {
                ...
                marketReceiveLoop = Task.Run(
                    () => ReceiveLoopAsync(marketSocket, PolymarketChannel.Market, activeMarketCts.Token),
            ...
            var keepAliveMarketCts = marketCts;
            marketPingLoop ??= Task.Run(
                () => PingLoopAsync(marketSocket, keepAliveMarketCts!.Token),
                CancellationToken.None);
        }
        finally
        {
        ...
        try
        {
            ...
            if (userSocket is null || userSocket.State != WebSocketState.Open)
            {
                ...
                userReceiveLoop = Task.Run(
                    () => ReceiveLoopAsync(userSocket, PolymarketChannel.User, activeUserCts.Token),
                    CancellationToken.None);
            }
            ...
            // Запуск ping/pong keepalive
            var keepAliveUserCts = userCts;
            userPingLoop ??= Task.Run(
                () => PingLoopAsync(userSocket, keepAliveUserCts!.Token),
                CancellationToken.None);
        }
        finally
        {
