...
public sealed class MarketStreamingPipeline : IMarketStreamingPipeline
{
    ...
    private readonly IWritableMarketPriceStream evaluationPriceStream;
    private readonly bool ownsEvaluationPriceStream;
        ...
        StreamingPipelineConfig? config = null)
    {
        ...
        if (priceStream is IWritableMarketPriceStream writablePriceStream)
        {
            evaluationPriceStream = writablePriceStream;
            ownsEvaluationPriceStream = false;
        }
        else
        {
            evaluationPriceStream = new PipelinePriceStream();
            ownsEvaluationPriceStream = true;
        }
    }
    ...
    private async Task EvaluatorLoopAsync(CancellationToken cancellationToken)
    {
        ...
        await foreach (var update in reader.ReadAllAsync(cancellationToken).ConfigureAwait(false))
        {
            ...
            evaluationPriceStream.SetPrice(update.AssetId, snapshot);
            ...
            foreach (var (strategy, assetIds) in currentStrategies)
            {
                ...
                var signal = strategy.Evaluate(evaluationPriceStream, update.AssetId);
