...
public class UnhandledErrorCollection
{
    ...
    /// </summary>
    public void ClearUnhandledErrors() => this.unhandledErrors.Clear();
    ...
    public bool HasUnhandledErrors(TransportErrorBehavior errorBehavior)
    {
        ...
        var behaviors = this.errorBehaviors
            .Where(pair => pair.Value == errorBehavior)
            .Select(pair => pair.Key).ToList();
        return this.unhandledErrors.Any(error => behaviors.Contains(error.ErrorType));
    }
}
