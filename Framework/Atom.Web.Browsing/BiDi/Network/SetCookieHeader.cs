...
public class SetCookieHeader
{
    ...
    {
        Name = name;
        Value = BytesValue.FromString(value);
    }
    ...
    public SetCookieHeader([NotNull] Cookie cookie)
    {
        Name = cookie.Name;
        Value = cookie.Value;
    ...
    [JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
    [JsonInclude]
    internal string? EpochExpires => Expires.HasValue ? $"{Expires.Value.ToUniversalTime():ddd, dd MMM yyyy HH:mm:ss} GMT" : null;
}