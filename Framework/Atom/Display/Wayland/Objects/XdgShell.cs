...
internal sealed class XdgSurface : WaylandObject
{
    ...
    public void SendInitialConfigure()
    {
        ...
        //
        // ★ Исключение — размер, названный композитору заранее (сейчас только для Firefox): тот на
        // ноль встаёт в свой минимум 500×200, потому что -width/-height на Wayland не действуют.
        // Это тот же заявленный при запуске размер, а не разрешение экрана.
        RequestedSize = Client.Compositor.InitialWindowSize;
        SendCurrentConfigure();
    }
