...
public sealed class Tls13ClientHandshake(in TlsSettings settings) : IDisposable
{
    ...
    /// Finished и обрывал соединение фатальным <c lang="text">unexpected_message</c> — уже ПОСЛЕ успешной
    /// проверки его собственного сертификата, то есть выглядело это как поломка на ровном месте.
    /// Замер на двух сотнях узлов: так были полностью недоступны <c lang="text">www.bund.de</c> (портал
    /// правительства Германии) и <c lang="text">www.zomato.com</c>, на всех трёх профилях сразу.
    ///
