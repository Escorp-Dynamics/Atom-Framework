...
/// • <c>0x009D</c> (RSA + AES-256-GCM) требует PRF на SHA-384. Проверка вида
///   «<c>suite is 0xC030 or 0xC02C</c>» его не знает и вернула бы SHA-256 — master secret вышел бы
///   неверным, а сервер отверг бы Finished. Выглядит это как «сервер сломался».
/// • тот же <c>0x009D</c> требует 32-байтовый ключ; перечисление «<c>0xC030 or 0xC02C or 0xCCA8…</c>»
