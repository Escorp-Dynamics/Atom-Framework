...
public static class ServerCertificateVerifier
{
    ...
    /// <c>GetNameInfo(X509NameType.DnsName)</c>, который возвращает лишь одну запись, — и
    ...
    /// <c>[*.example.com, example.com]</c>, и обращение к голому <c>example.com</c> не совпадало
    /// с шаблоном <c>*.example.com</c>, потому что шаблон покрывает только поддомены.
    ///
    ...
    ///
    /// Адреса из SAN тоже учитываются: обращение к <c>https://1.2.3.4/</c> сверяется с записями
