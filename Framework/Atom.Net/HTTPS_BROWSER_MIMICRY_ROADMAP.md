...
## Live Capture (2026-08-28)

Живой capture Chromium 151 (headless --dump-dom на loopback capture-сервер) опроверг часть
закодированных предположений; wire-truth внесена в активный путь:

- Sec-Fetch-* на h1 идут в Title-Case (Sec-Fetch-Site), НЕ в нижнем регистре;
- Priority-заголовок на h1-навигации НЕ отправляется (только h2/h3);
- sec-ch-* остаются в нижнем регистре;
- Accept-Language живого браузера — системная локаль с цепочкой fallback (ru-RU,ru;q=0.9,...),
...
1. Кэш билетов хранит и последний согласованный ALPN узла; 0-RTT предлагается только когда прошлый
   протокол был h2 (иначе ранние байты легли бы в h1.1-парсер и дали бы ошибку соединения).
2. Только безопасные запросы (GET/HEAD) без тела; payload = преамбула h2 + SETTINGS профиля +
   HEADERS(END_STREAM), собранные ДО рукопожатия (Https2Connection.BuildEarlyRequestPayload);
   кодировщик payload'а передаётся сеансу — динамическая таблица HPACK продолжается без рассинхрона.
3. После рукопожатия: при подтверждении (early_data в EE) сеанс стартует БЕЗ префейса и
   регистрирует поток 1 как уже отправленный; при отказе — обычный путь отправки.
4. Тесты: валидность payload (кадры, END_STREAM, поток 1), сеанс без префейса с ответом на поток 1.
