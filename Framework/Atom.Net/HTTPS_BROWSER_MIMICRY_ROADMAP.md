...
#### Next Task: автоматический 0-RTT на уровне handler'а (H2)
...
2. Только безопасные запросы (GET/HEAD) без тела; payload = преамбула h2 + SETTINGS профиля +
   HEADERS(END_STREAM), собранные ДО рукопожатия теми же сборщиками (Http2Preface, Http2RequestHeaders,
   HPackEncoder с пустой таблицей).
3. Передача payload в Tls13Stream до HandshakeAsync через options; после рукопожатия:
   при принятом 0-RTT Http2Session стартует без префейса и регистрирует поток 1 как уже
   отправленный (ответ читается штатно); при отказе — обычный путь отправки.
4. Регрессионный gate: все H2-тесты + новый тест «0-RTT принят → ответ на ранний запрос;
   0-RTT отвергнут → запрос переотправлен обычным путём».
