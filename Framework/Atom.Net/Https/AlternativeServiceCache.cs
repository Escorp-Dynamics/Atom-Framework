...
/// <summary>
/// Запоминает объявленные сервером альтернативные службы (RFC 7838, заголовок <c>alt-svc</c>).
/// </summary>
/// <remarks>
...
/// никуда. Поэтому браузер сначала идёт по HTTP/2, читает в ответе <c>alt-svc: h3=":443"</c> и
...
/// замер на <c>cloudflare-quic.com</c> даёт 292 мс по HTTP/3 против 586 мс по HTTP/2.
///
/// Хранится не адрес, а факт поддержки и срок его действия: <c>ma</c> задаёт время жизни записи в
...
public sealed class AlternativeServiceCache
{
    ...
    /// <param name="headerValue">Значение заголовка <c>alt-svc</c>.</param>
    ...
    /// заголовок <c>alt-svc</c> приходит в ответе по HTTP/2 и тут же записывает его обратно.
    ...
    ///
    /// Замер на <c>www.nature.com</c> (объявляет <c>h3</c>, но UDP до него не доходит):
    ...
    /// <remarks>
    /// Заголовок перечисляет службы через запятую, каждая — вида <c>протокол="узел:порт"</c> с
    /// необязательными параметрами. Нас интересует только протокол <c>h3</c>; его черновые
    /// версии (<c>h3-29</c> и подобные) намеренно НЕ принимаются: мы говорим на окончательной.
    /// </remarks>
