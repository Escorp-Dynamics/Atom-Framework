...
/// недосмотру: профиль браузера добавляет <c>connection: keep-alive</c> для HTTP/1.1, где он
...
public static class ConnectionHeaderRules
{
    ...
    /// <returns><see langword="true"/>, если заголовок передавать нельзя.</returns>
    /// <remarks>
    /// Помимо списка соединения отсекаются ещё два случая. Устаревший <c>host</c> рядом с
    /// <c>:authority</c> — тоже некорректное сообщение (RFC 9113 §8.3.1). И <c>te</c>: он
    /// единственное исключение из списка соединения, но допустим ровно с одним значением —
    /// <c>trailers</c>; любое другое снова делает запрос малформированным.
    /// </remarks>
