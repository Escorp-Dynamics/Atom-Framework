...
export class BackgroundRuntimeHost {
    ...
    private async attemptTransportReconnect(reason: string): Promise<void> {
        ...
        // Сервер закрывает кодом 1008 «идентификатор-сеанса-уже-занят» ТОЛЬКО вытесняемый
        // сокет: значит, сеанс уже занят конкурирующим коннектом (перезапущенный service
        // worker, параллельный инстанс после гибели браузерного процесса). Продолжать
        // переподключение из этого инстанса нельзя — иначе два инстанса вытесняют друг
        // друга по кругу (500 мс каждый), пока браузер не умрёт. Уступаем: выигравший
        // остаётся подключённым, а этот инстанс молчит, пока его снова не разбудит событие.
        if (reason.includes('идентификатор-сеанса-уже-занят')) {
            this.transportReconnectAttempts = 0;
            emitBackgroundDebugEvent(this.config, 'transport-reconnect-yielded', {
                reason,
            });
            return;
        }
