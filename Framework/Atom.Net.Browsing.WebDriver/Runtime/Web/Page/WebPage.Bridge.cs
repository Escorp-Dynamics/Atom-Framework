...
public sealed partial class WebPage
{
    ...
    /// <summary>
    /// TLS-профиль переотправки, привязанный к личности ТЕКУЩЕЙ задачи вкладки.
    /// </summary>
    /// <remarks>
    /// Заполняется вызывающим (например, пулом слотов солвера) при переприменении контекста
    /// вкладки под новую задачу. nav-proxy забирает его в маршрут при регистрации и переотправляет
    /// трафик этой задачи согласованным с задачей отпечатком, а не профилем запуска браузера.
    /// </remarks>
    internal Atom.Net.Https.Profiles.BrowserProfile? BridgeTaskForwardProfile { get; set; }
