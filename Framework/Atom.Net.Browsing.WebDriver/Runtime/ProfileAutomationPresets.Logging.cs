...
internal static partial class ProfileAutomationPresetLogs
{
    ...
    [LoggerMessage(EventId = 1908, Level = LogLevel.Warning, Message = "Профиль устройства заявляет движок Chromium (агент: {UserAgent}), а запускается Firefox. Gecko не отдаёт ни клиентских подсказок, ни navigator.userAgentData, ни объекта chrome, а формат исходника нативных функций и тексты исключений у него свои — подмена свойств этого не закрывает. Замер на живом Cloudflare Turnstile: 0 из 4, error-callback 600010. Обратное направление (профиль Firefox на Chromium) работает: 3 из 4")]
    public static partial void LogProfileAutomationChromiumEngineMismatch(this ILogger logger, string userAgent);
}