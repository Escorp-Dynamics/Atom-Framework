...
/// <summary>
/// Высокооптимизированный видеокодек WebM (VP9).
/// </summary>
/// <remarks>
/// <para>
/// Поддерживает Store mode для zero-overhead round-trip с использованием
/// собственного AFRM (Atom Frame) chunk внутри Matroska контейнера.
/// </para>
/// <para>
/// Форматы пикселей: YUV420P, YUV422P, YUV444P, RGB24, RGBA32.
/// </para>
/// <para>
/// Оптимизации:
/// <list type="bullet">
...
/// </para>
/// </remarks>
public sealed partial class WebmCodec : IVideoCodec
{
    #region Constants

    /// <summary>Минимальный размер заголовка EBML.</summary>
    ...
    /// <inheritdoc/>
    public MediaCodecId CodecId { get; private set; } = MediaCodecId.Vp9;
    ...
    public static int EstimateEncodedSize(int width, int height, VideoPixelFormat format)
    {
        ...
        // AFRM header: magic(4) + width(4) + height(4) + format(4) + frameIndex(8) + flags(4) = 28 bytes
        // + EBML overhead (~64 bytes max)
        return frameSize + 128;
    }
    ...
    /// <summary>
    /// Проверяет, является ли данные WebM потоком.
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
