...
public static class Extensions
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static object? GetParameter([NotNull] this AttributeData attribute, [NotNull] string name, Type type)
    {
        var typeName = type.GetFriendlyName(default).TrimEnd('?');
        ...
        if (!constructorArg.IsNull) return constructorArg.Value;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static T? GetParameter<T>([NotNull] this AttributeData attribute, [NotNull] string name) => attribute.GetParameter(name, typeof(T)) is T v ? v : default;
}